...
public class Laser_facingup : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        RaycastHit2D hit = Physics2D.Raycast(transform.position, Vector2.up, 50f, layersToHit);
        if(hit.collider == null){
            transform.localScale = new Vector3(.5f, 50f, 1);
            return;
        }
        transform.localScale = new Vector3(.5f, hit.distance + 0.25f, 1);
